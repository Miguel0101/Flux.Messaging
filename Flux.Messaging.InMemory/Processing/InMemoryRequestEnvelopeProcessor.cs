...
internal sealed class InMemoryRequestEnvelopeProcessor(
    ...
    [FromKeyedServices(MessagingProviders.InMemory)] IPendingRequestsManager pendingRequests) : IEnvelopeProcessor
{
    ...
    public async Task ProcessAsync(MessageEnvelope envelope, CancellationToken ct = default)
    {
        ...
        {
            if (pendingRequests.TryRemove(envelope.Id, out var tcs))
            {
